...
summary_dat <- dat %>% select(SPARK, Region, Vegetation, Total_area,
                              Deficit_area, Surplus_area, mean_deficit,
                              sd_deficit) %>%
...
#for (spark in unique(graph_dat$SPARK)) {
#  graphs_list[[spark]] <- 
#    ggplot(data = graph_dat %>% filter(SPARK == spark), 
#           aes(y = Region, x = Area, fill = Vegetation)) +
#    geom_bar(stat='identity') + #, orientation = 'y') +
#    scale_fill_manual(values = veg_levels, name="") +
#    labs(x = "Relative area", y = "County", 
...
#    theme_minimal()
#}
#patchwork::wrap_plots(graphs_list) +
#  plot_layout(ncol = 1, axis_titles = "collect", guides="collect") 
