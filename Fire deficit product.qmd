...
suppressMessages(region_plot <- 
  ...
  ylim(c(min(summary_dat$def_lower) - 0.5, 10)) +
  labs(
...
![](figs/WY - Upper Colorado Basin, Green River4.png)
...
state_names <- c("Arizona" = "AZ", "California" = "CA", "Colorado" = "CO",
                 "Idaho" = "ID", "Kansas" = "KS", "Montana" = "MT",
                 "New Mexico" = "NM", "Nevada" = "NV", "Oklahoma" = "OK",
...
We use this dataset by turning it into a raster of times each pixel has burned.
...
The number of expected fires $N_{exp}$ in the 40 years of our burn data is $40/FRI$, and call $N_{act}$ the number of observed fires from the dataset. For surpluses, we take $N_{act}/N_{exp}$; for deficits, we take the inverse of that, and add a negative. (If an area has not burned ($N_{act}=0$), deficit will be equal to negative expected fires.)

```{r create-fire-deficit-raster}
#| eval: false

#----- Actual number of fires ------------------------------------------------#
