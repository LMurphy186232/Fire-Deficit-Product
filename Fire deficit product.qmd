...
**Output:** rasters called "west_us_FRI40.tif" and "west_us_FRI33.tif", of CONUS cropped to the WFFRC study area, with a map of Landfire's 2016 FRI_ALLFIR values where FRI_ALLFIR is $\leq$ 40 or $\leq$ 33, respectively. (See appendix for summary Landfire documentation.)

```{r create-western-us-fri-rast}
#| eval: FALSE

for (threshold in c(33, 40)) {
...
fri <- terra::rast("../Rasters FRI 33/west_us_FRI33.tif")
plot(fri, axes=F,
     main="Fire return interval where values <= 33 years")
plot(states, add=T, fill=NULL)
```
...
def <- terra::rast(paste0("../Rasters FRI 33/western_us_fire_deficit33.tif"),)
plot(def, axes=F,
     range=c(-5,5),
     fill_range=T,
     col=map.pal("bgyr"),#, bias=2.5),
     main="Fire deficit / surplus based on FRI 33")
plot(states, add=T, fill=NULL)
rm(def)
invisible(gc())
```
...
**Output:** rasters called "forest_deficit40.tif" (where FRI $\leq$ 40 years) and "forest_deficit33.tif" (where FRI $\leq$ 33 years), where the values in the raster "western_us_fire_deficitX.tif" are masked out to areas in our forest raster ("western_us_forest.tif").
...
defi <- terra::rast("../Rasters FRI 33/western_us_fire_deficit33.tif")

fordef <- terra::mask(defi, fors)
writeRaster(fordef, "../Rasters FRI 33/forest_deficit33.tif", overwrite=T)
```
...
fordef <- terra::rast("../Rasters FRI 33/western_us_fire_deficit33.tif")
plot(fordef, axes=F,
     range=c(-5,5),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
     main="Forest area FRI 33 fire deficit / surplus")
plot(states, add=T, fill=NULL)
rm(fordef)
invisible(gc())
```
...
**Output:** rasters called "grass_deficit40.tif" (where FRI $\leq$ 40 years) and "grass_deficit33.tif" (where FRI $\leq$ 33 years), where the values in the raster "western_us_fire_deficitX.tif" are masked out to areas in our grass raster ("western_us_grass.tif").
...
defi <- terra::rast("../Rasters FRI 33/western_us_fire_deficit33.tif")
fordef <- terra::mask(defi, fors)
writeRaster(fordef, "../Rasters FRI 33/grass_deficit33.tif", overwrite=T)
```
...
fordef <- terra::rast("../Rasters FRI 33/grass_deficit33.tif")
plot(fordef, axes=F,
     range=c(-5,2),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
     main="Grass area FRI 33 fire deficit / surplus")
plot(states, add=T, fill=NULL)
rm(fordef)
invisible(gc())
```
...
**Output:** rasters called "shrub_deficit40.tif" (where FRI $\leq$ 40 years) and "shrub_deficit33.tif" (where FRI $\leq$ 33 years), where the values in the raster "western_us_fire_deficitX.tif" are masked out to areas in our shrub raster ("western_us_shrub.tif").
...
defi <- terra::rast("../Rasters FRI 33/western_us_fire_deficit33.tif")
fordef <- terra::mask(defi, fors)
writeRaster(fordef, "../Rasters FRI 33/shrub_deficit33.tif", overwrite=T)
```
...
fordef <- terra::rast("../Rasters FRI 33/shrub_deficit33.tif")
plot(fordef, axes=F,
     range=c(-5,5),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
     main="Shrub area FRI 33 fire deficit / surplus")
plot(states, add=T, fill=NULL)
rm(fordef)
invisible(gc())
```
...
When considering the area *A* and the burned areas $a_i$, I am only considering pixels within the subsetted areas: within the ecoregion, within those areas where FRI \> 40 years or FRI \> 33 years (to complement the two possible definitions of frequent fire areas1), within the vegetation type in question. Same when calculating mean FRI.
...
fri <- terra::rast("../Rasters FRI 33/west_us_long_FRI33.tif")
plot(fri, axes=F, 
     main="Study areas with FRI > 33 years, with ecoregions")
plot(eco, add=T, border="red")
```
