...
for (spark_name in all_sparks) {
  ...
  fri_crop <- terra::crop(fri     , one_spark)
  fri_crop <- terra::mask(fri_crop, one_spark)
  
  #plot(fri_crop)
  
  suppressMessages(fri_plot <- ggplot() + 
                     geom_spatraster(data = fri_crop) +
                     geom_spatvector(data = one_spark, fill=NA) +
                     theme_bw() +
                     blank_axes +
  ...
  veg_crop <- terra::crop(veg     , one_spark)
  veg_crop <- terra::mask(veg_crop, one_spark)
  ...
  long_crop <- terra::crop(long_forst_def, one_spark)
  long_crop <- terra::mask(long_crop     , one_spark)
  
  short_crop <- terra::crop(short_def , one_spark)
  short_crop <- terra::crop(short_crop, one_spark)
  ...
  #----- Make a total fire deficit map, including long deficit -----------------#
  long_crop <- terra::crop(long_grass_def, one_spark)
  long_crop <- terra::mask(long_crop     , one_spark)
  ...
  #----- Make a total fire deficit map, including long deficit -----------------#
  long_crop <- terra::crop(long_shrub_def, one_spark)
  long_crop <- terra::mask(long_crop     , one_spark)
  ...
  spark_name <- gsub(spark_name, "/", "-", fixed=T)
  ggsave(paste0(spark_name, "1.png"), plot= print(fri_plot + veg_map_plot), 
         width = 8, units="in")
  ggsave(paste0(spark_name, "2.png"), plot= print(forest_types_plot + fire_def_plot), 
         width = 8, units="in")
  ggsave(paste0(spark_name, "3.png"), plot= print(grassplot + shrubplot), 
         width = 8, units="in")
...
#----- Load the BPS raster ---------------------------------------------------#
fri <- terra::rast("Z:/project_data/landfire data/LF2016_FRI_200_CONUS/Tif/LC16_FRI_200.tif")
fri_crs <- crs(fri)
...
#----- terra::project to the FRI raster. This will mess up the study domain extent --#
...
akfri <- terra::rast("Z:/project_data/landfire data/LF2023_FRI_240_AK/Tif/LA23_FRI_240.tif")
```
...
fire_perimeters <- terra::vect("Z:/Mahsa_GF_data/goodfire_data_jan25/goodfire_1985to2024_f1jan25.shp")
...
#| eval: false
nfire <- terra::rast("../temp_rasters/num_fires.tif")
fri <- terra::rast("../west_us_FRI.tif")
...
#----- Load the BPS raster ---------------------------------------------------#
evc <- terra::rast("Z:/project_data/landfire data/LF2016_EVC_200_CONUS/Tif/LC16_EVC_200.tif")
...
#----- Load the BPS raster ---------------------------------------------------#
frg <- terra::rast("Z:/project_data/landfire data/LF2016_FRG_200_CONUS/Tif/LC16_FRG_200.tif")
study_domain <- terra::rast("../temp_rasters/study_domain.tif")
...
This is the contemporary FRP. We can take the mean ecoregion FRI as the historical value. Ecoregion-level deficit / surplus will thus be contemporary divided by historical.
...
terra::writeVector(eco, "../temp_rasters/ecoregions_with_deficit.shp")

#----- Now rasters of all these ----------------------------------------------#
def_rast <- rast(fri)
allvegrast <- rasterize(eco, def_rast, field = "allveg_def", touches = TRUE)
terra::writeRaster(allvegrast, "../temp_rasters/eco_all_veg_def.tif")
rm(rasterized)

forestrast <- rasterize(eco, def_rast, field = "forest_def", touches = TRUE)
terra::writeRaster(forestrast, "../temp_rasters/eco_forest_def.tif")

grassrast <- rasterize(eco, def_rast, field = "grass_def", touches = TRUE)
terra::writeRaster(grassrast, "../temp_rasters/eco_grass_def.tif")

shrubrast <- rasterize(eco, def_rast, field = "shrub_def", touches = TRUE)
terra::writeRaster(shrubrast, "../temp_rasters/eco_shrub_def.tif")
