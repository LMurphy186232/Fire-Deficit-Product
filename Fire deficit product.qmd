...
WUI data was downloaded from [https://silvis.forest.wisc.edu/maps-data/wildland-urban-interface/]( https://silvis.forest.wisc.edu/maps-data/wildland-urban-interface/). The reference is [Carlson, A.R., Helmers, D.P., Hawbaker, T.J., Mockrin, M.H., and V. C. Radeloff. 2022. The wildland–urban interface in the United States based on 125 million building locations. Ecological Applications, 2022;32:e2597.](https://silvis.forest.wisc.edu/wp-content/uploads/sites/379/2023/03/Carlson-Ecological-Applications-2022-Carlson-The-wildland-urban-interface-in-the-United-States-based-on-125-million-building.pdf)

There are several possible ways to define the WUI. I am using the product derived from building footprints, with a neighborhood scale of 500 m. From Carlson et al: 

> We determined that the neighborhood with a 500-m radius therefore most closely matches commonly accepted definitions of the WUI for wildfire resource allocation and planning based on communities rather than individual structures (Wildland Fire Executive Council, 2014), while still providing a high level of spatial detail around building locations. We focus on maps produced using this neighborhood size as our recommended products for wildfire planning and for general purposes.
...
#----- Load WUI --------------------------------------------------------------#
wui <- terra::vect("Z:/project_data/western_fire/wui/CONUS_WUI_structures_MSBFP.gdb", layer="CONUS_wui_poly_final_500")
...
writeRaster(wui_rast, "../temp_rasters/wui.tif", overwrite=T)
```

# Recent burns - not prescribed
...
#----- FRI being used solely for its geospatial characteristics, not values --#
...
rx_rast <- rasterize(rx2, rast, fun="count", background = 0)
writeRaster(rx_rast, "../temp_rasters/num_rx.tif", overwrite = T)
```


# Landfire historical fire regime / fire return interval

We are using the fire return interval (FRI) to separate the western US into frequent fire and infrequent fire areas. For the frequent fire areas, we have the ability to create a finer resolution spatially-explicit map of fire deficit/surplus.

When we started this project, we defined "frequent fire" as having an FRI of 40 years or less, which was the length of our observed fire records. But with a more conservative definition of frequent fire, allowing for more of a buffer before an area tips over into deficit, we also want to look at a definition of "frequent fire" as FRI $\leq$ 32 years (20% margin).
...
**Output:** rasters called "west_us_FRI40.tif" and "west_us_FRI32.tif", of CONUS cropped to the WFFRC study area, with a map of Landfire's 2016 FRI_ALLFIR values where FRI_ALLFIR is $\leq$ 40 or $\leq$ 32, respectively. (See appendix for summary Landfire documentation.)

```{r create-western-us-fri-rast}
#| eval: FALSE

for (threshold in c(32, 40)) {
  ...
  # longer requires tiles. I am leaving this in as a legacy, in case it is 
  # useful later; but you can go straight to the classify step if desired.
  #---------------------------------------------------------------------------#
  ...
  #----- Filter out those units with an all-fire FRI <= threshold ------------#
  good_bps <- which(bps$FRI_ALLFIR > 0 & bps$FRI_ALLFIR <= threshold) 
  ...
  mosaic <- classify(fri_crop, mat, others=NA)
  
  writeRaster(mosaic, paste0("../Rasters FRI ", threshold, "/west_us_FRI",
                             threshold, ".tif"), overwrite = TRUE)
}
```
...
fri <- terra::rast("../Rasters FRI 40/west_us_FRI40.tif")
...
fri <- terra::rast("../Rasters FRI 32/west_us_FRI32.tif")
plot(fri, axes=F,
     main="Fire return interval where values <= 32 years")
plot(states, add=T, fill=NULL)
```

\newpage

# Fire deficit / surplus for frequent fire areas

**Output:** raster called "western_us_fire_deficit40.tif" or "western_us_fire_deficit32.tif". The value in each raster is the deficit or surplus. Values above zero are surplus, and represent the number of times more an area has burned than it should have (i.e. a value of 2 means the area has burned twice as many times as expected). Values below zero are deficit, and are interpretable in the same way (-2 means the area should have burned twice as much as it did).

The number of expected fires $N_{exp}$ is either $40/FRI$ or $32/FRI$, and call $N_{act}$ the number of observed fires from the dataset (wild plus prescribed burns). The fire balance (either deficit or surplus) is calculated as follows:

$$balance = \begin{cases}
        N_{act}/N_{exp} & if N_{act} \geq N_{exp}  \\
        -N_{exp}/N_{act}& if N_{exp} > N_{act}
        \end{cases}$$
        
If an area has not burned ($N_{act}=0$), deficit will be equal to negative expected fires.

For comparison, there is a separate raster called "western_us_fire_deficit_norx.tif", based on a frequent fire FRI threshold of 40 years, that does not contain prescribed burns.
...
for (threshold in c(32, 40)) {
  ...
  fri <- terra::rast(paste0("../Rasters FRI ", threshold, "/west_us_FRI",
                             threshold, ".tif"))
  n_exp <- threshold/fri
  ...
  writeRaster(def, paste0("../Rasters FRI ", threshold, 
                          "/western_us_fire_deficit", threshold, ".tif"), 
              overwrite=T)
}
```

```{r}
def <- terra::rast(paste0("../Rasters FRI 40/western_us_fire_deficit40.tif"),)
...
plot(def, axes=F,
     ...
     main="Fire deficit / surplus based on FRI 40")
plot(states, add=T, fill=NULL)
rm(def)
invisible(gc())

def <- terra::rast(paste0("../Rasters FRI 32/western_us_fire_deficit32.tif"),)
plot(def, axes=F,
     range=c(-5,5),
     fill_range=T,
     col=map.pal("bgyr"),#, bias=2.5),
     main="Fire deficit / surplus based on FRI 32")
plot(states, add=T, fill=NULL)
rm(def)
invisible(gc())
```
...
## Frequent fire forest area deficit/surplus

**Output:** rasters called "forest_deficit40.tif" (where FRI $\leq$ 40 years) and "forest_deficit32.tif" (where FRI $\leq$ 32 years), where the values in the raster "western_us_fire_deficitX.tif" are masked out to areas in our forest raster ("western_us_forest.tif").

```{r create-forest-deficit-rast}
#| eval: false

fors <- terra::rast("../western_us_forest.tif")
defi <- terra::rast("../Rasters FRI 40/western_us_fire_deficit40.tif")

fordef <- terra::mask(defi, fors)
writeRaster(fordef, "../Rasters FRI 40/forest_deficit40.tif", overwrite=T)

defi <- terra::rast("../Rasters FRI 32/western_us_fire_deficit32.tif")

fordef <- terra::mask(defi, fors)
writeRaster(fordef, "../Rasters FRI 32/forest_deficit32.tif", overwrite=T)
```

```{r}
fordef <- terra::rast("../Rasters FRI 40/western_us_fire_deficit40.tif")
plot(fordef, axes=F,
     range=c(-5,5),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
     main="Forest area FRI 40 fire deficit / surplus")
plot(states, add=T, fill=NULL)
rm(fordef)
invisible(gc())

fordef <- terra::rast("../Rasters FRI 32/western_us_fire_deficit32.tif")
plot(fordef, axes=F,
     range=c(-5,5),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
     main="Forest area FRI 32 fire deficit / surplus")
plot(states, add=T, fill=NULL)
rm(fordef)
invisible(gc())
```
...
**Output:** rasters called "grass_deficit40.tif" (where FRI $\leq$ 40 years) and "grass_deficit32.tif" (where FRI $\leq$ 32 years), where the values in the raster "western_us_fire_deficitX.tif" are masked out to areas in our grass raster ("western_us_grass.tif").
...
defi <- terra::rast("../Rasters FRI 40/western_us_fire_deficit40.tif")
fordef <- terra::mask(defi, fors)
writeRaster(fordef, "../Rasters FRI 40/grass_deficit.tif", overwrite=T)

defi <- terra::rast("../Rasters FRI 32/western_us_fire_deficit32.tif")
fordef <- terra::mask(defi, fors)
writeRaster(fordef, "../Rasters FRI 32/grass_deficit32.tif", overwrite=T)
```

```{r}
fordef <- terra::rast("../Rasters FRI 40/grass_deficit40.tif")
plot(fordef, axes=F,
     range=c(-5,2),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
     main="Grass area FRI 40 fire deficit / surplus")
plot(states, add=T, fill=NULL)
rm(fordef)
invisible(gc())

fordef <- terra::rast("../Rasters FRI 32/grass_deficit32.tif")
plot(fordef, axes=F,
     range=c(-5,2),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
     main="Grass area FRI 32 fire deficit / surplus")
plot(states, add=T, fill=NULL)
rm(fordef)
invisible(gc())
```
...
**Output:** rasters called "shrub_deficit40.tif" (where FRI $\leq$ 40 years) and "shrub_deficit32.tif" (where FRI $\leq$ 32 years), where the values in the raster "western_us_fire_deficitX.tif" are masked out to areas in our shrub raster ("western_us_shrub.tif").
...
defi <- terra::rast("../Rasters FRI 40/western_us_fire_deficit40.tif")
fordef <- terra::mask(defi, fors)
writeRaster(fordef, "../Rasters FRI 40/shrub_deficit40.tif", overwrite=T)

defi <- terra::rast("../Rasters FRI 32/western_us_fire_deficit32.tif")
fordef <- terra::mask(defi, fors)
writeRaster(fordef, "../Rasters FRI 32/shrub_deficit32.tif", overwrite=T)
```

```{r}
fordef <- terra::rast("../Rasters FRI 40/shrub_deficit40.tif")
plot(fordef, axes=F,
     range=c(-5,5),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
     main="Shrub area FRI 40 fire deficit / surplus")
plot(states, add=T, fill=NULL)
rm(fordef)
invisible(gc())

fordef <- terra::rast("../Rasters FRI 32/shrub_deficit32.tif")
plot(fordef, axes=F,
     range=c(-5,5),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
     main="Shrub area FRI 32 fire deficit / surplus")
plot(states, add=T, fill=NULL)
rm(fordef)
invisible(gc())
```
...
# Fire deficit / surplus for infrequent fire areas
...
This is the contemporary FRP. We can take the mean ecoregion FRI as the historical value. Ecoregion-level deficit / surplus will thus be contemporary divided by historical if contemporary is greater (deficit), or negative historical divided by contemporary if historical is greater (surplus). This makes the number interpretable in the same way as calculated for more frequent fire areas.

When considering the area *A* and the burned areas $a_i$, I am only considering pixels within the subsetted areas: within the ecoregion, within those areas where FRI \> 40 years or FRI \> 32 years (to complement the two possible definitions of frequent fire areas1), within the vegetation type in question. Same when calculating mean FRI.
...
for (threshold in c(32, 40)) {
  ...
  #----- Filter out those units with an all-fire FRI <= the threshold --------#
  good_bps <- which(bps$FRI_ALLFIR > threshold) 
  ...
  writeRaster(fri, paste0("../Rasters FRI ", threshold, 
                          "/west_us_long_FRI", threshold, ".tif"))
}
```

```{r}
eco <- terra::vect("../Data/level_3_ecoregion/eco_region.shp")
fri <- terra::rast("../Rasters FRI 40/west_us_long_FRI40.tif")
...
fri <- terra::rast("../Rasters FRI 32/west_us_long_FRI32.tif")
plot(fri, axes=F, 
     main="Study areas with FRI > 32 years, with ecoregions")
plot(eco, add=T, border="red")
```
...
for (threshold in c(32, 40)) {
  fri <- terra::rast(paste0("../Rasters FRI ", threshold, "/west_us_long_FRI",
                            threshold, ".tif"))
  if (threshold == 32) {
    eco <- terra::project(eco, fri)
    states <- terra::project(states, fri)
  ...
  forest_dat <- data.frame(Name = eco$NA_L3NAME, 
                           ...
                           Total_Area = vv,
                           Area_Burned = v2)
  write.csv(forest_dat, paste0("../Rasters FRI ", threshold,
                               "/all_veg_cells_by_ecoregion_long_FRI",
  ...
  forest_dat <- data.frame(Name = eco$NA_L3NAME, 
                           ...
                           Area_Burned = v2)
  write.csv(forest_dat, paste0("../Rasters FRI ", threshold,
                               "/forest_cells_by_ecoregion_long_FRI",
                               threshold, ".csv"), row.names = F)
  ...
  write.csv(grass_dat, paste0("../Rasters FRI ", threshold,
                               "/grass_cells_by_ecoregion_long_FRI",
                               threshold, ".csv"), row.names = F)
  ...
  shrub_dat <- data.frame(Name = eco$NA_L3NAME, 
                          ...
                          Sum_FRI = sum_fri,
                          Area_Burned = v2)
  write.csv(shrub_dat, paste0("../Rasters FRI ", threshold,
                               "/shrub_cells_by_ecoregion_long_FRI",
                               threshold, ".csv"), row.names = F)
  rm(shrub, good_shrub)
  gc()
...
dat <- all_veg_dat %>% group_by(Name) %>% 
  ...
  mutate(Deficit = ifelse (FRP > Mean_FRI, -1 * FRP / Mean_FRI, Mean_FRI / FRP)) %>%
