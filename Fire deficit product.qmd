...
writeRaster(study_domain, "study_domain.tif")
...
#----- Save our work here ---------------------------------------------------#
writeRaster(fri_crop, "fri_crop.tif")
rm(fri_crop)
gc()
```
...
writeRaster(r_count, "num_fires.tif")
```
```{r create-fire-deficit-raster}
#| eval: false
nfire <- rast("num_fires.tif")
fri <- rast("west_us_FRI.tif")
...
**Output:** raster called "west_us_forest.tif"
...
study_domain <- rast("study_domain.tif")
...
#----- Save our work here ----------------------------------------------------#
writeRaster(evc_crop, "evc_crop.tif")
rm(evc_crop)
gc()
```
...
writeRaster(lcms_crop, "lcms_crop.tif")
```
...
evc_crop <- rast("evc_crop.tif")
evc <- classify(evc_crop, mat, others=NA)
rm(evc_crop)
gc()
writeRaster(evc, "evc_forest.tif", overwrite = TRUE)
```
```{r create-lcms-western-us-forest-rast}
#| eval: false
lcms_crop <- rast("lcms_crop.tif")
lcms_crop <- classify(lcms_crop, cbind(1,1), others=NA)
writeRaster(lcms_crop, "lcms_forest.tif", overwrite = TRUE)
```
...
rm(evc_crop)
gc()
writeRaster(evc, "evc_grass.tif", overwrite = TRUE)
```
...
writeRaster(lcms_crop, "lcms_grass.tif", overwrite = TRUE)
```
```{r create-western-us-grass-rast}
#| eval: false
evc   <- rast("evc_grass.tif")
lcms  <- rast("lcms_grass.tif")
...
rm(evc_crop)
gc()
writeRaster(evc, "evc_shrub.tif", overwrite = TRUE)
```
...
writeRaster(lcms_crop, "lcms_shrub.tif", overwrite = TRUE)
```
```{r create-western-us-shrub-rast}
#| eval: false
evc   <- rast("evc_shrub.tif")
lcms  <- rast("lcms_shrub.tif")
...
evc <- rast("evc_crop.tif")
evc_out <- mask(evc, all, inverse=T)
writeRaster(evc_out, "evc_excluded.tif")
...
lcms <- rast("lcms_crop.tif")
lcms_out <- mask(lcms, all, inverse=T)
writeRaster(lcms_out, "lcms_excluded.tif")
...
writeRaster(lcpri_crop, "lcpri_crop.tif")
```

```{r create-lcpri-western-us-forest-rast}
#| eval: false
lcpri_crop <- rast("lcpri_crop.tif")
lcpri_crop <- classify(lcpri_crop, cbind(4,1), others=NA)
writeRaster(lcpri_crop, "lcpri_forest.tif")
```
...
writeRaster(all, "forest_tally.tif")
```

```{r}
all <- rast("forest_tally.tif")
...
rm(evc_crop)
gc()
writeRaster(evc, "evc_forest_loose.tif", overwrite = TRUE)
```

```{r}
evc <- rast("evc_forest_loose.tif")
#x <- freq(evc)
#evc_forest_area <- (x$count * prod(res(evc)))/10000
...
> Hi Lora, Thanks so much for sharing this. Really nice. Now that you have all fire perimeters, I think we are ready to move forward. So heres what I think the next steps are: 
...
> 5. Then we need to know what the deficit vs surplus is for forests with longer FRIs than the observed record. We can't do that at the pixel scale cuz we need to use space for time substitution. So at the level 3 ecoregion level we calculate the fire rotation by veg type (forest, grassland, shrub) the historical fire rotation is the mean fire return interval. The observed fire rotation is calculated as we calculated fire rotation periods (FRPs), defined as the time it takes to burn an area equal in size to the study area:
 t/∑ai/A
 , where t is the time period evaluated, ai is annual area burned in year i, and A is the size of the study area.
...
> A few comments here:
> \[...\] I'm supportive of your classification strategies for veg type. 
...
> Once you create the final grassland, shrubland, forest raster layers, could you please:
Further break forest into dry frequent fire forests, mixed severity fire forest, and long interval stand replacing fire forests by using the following groupings 
This can be accomplished using the FRG_New variable in the BPS table. 
Dry frequent fire forests would be group 1 A-1C
Mixed severity fire regimes would be group IIIA-IIIB
Long interval stand replacing fire forests: IVA-IVB and potentiallly with VA and VB. I want to see how this looks including only the group IV categories alone and when IV and V categories are grouped together.
Once I look at those maps, we can make adjustments. Then I'd like you to send the grassland, shrubland, dry frequent fire forests, mixed severity forests, and high severity forest rasters to Mahsa and Crystal for another paper we are working on.
...
> Then at the ecoregion III level that includes all forest types, 
> we could run DYNAFFOREST to compare during the observational period and run the model into the future to determine when different ecoregions cross the threshold from deficit to surplus. 
