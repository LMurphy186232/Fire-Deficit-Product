...
```{r make-all-fri-rast}
#| eval: false
fri <- rast("../temp_rasters/fri_crop.tif")
...
tile <- classify(fri, mat, others=NA)
...
A value of 1 indicates forest according to our definition, NA for all others. Our definition of forest is based on a combination of [Landfire EVC](https://landfire.gov/vegetation/evc) 2016 and the Landscape Change Monitoring System (LCMS) 2016 data. I received LCMS data from Mahsa.

EVC forest cover classes:

-   Developed-upland deciduous forest - always included
-   Developed-upland evergreen forest - always included
-   Developed-upland mixed forest - always included
-   Tree cover - 20% to 100% - included if LCMS also categorizes the area as TREES.
...
#-----------------------------------------------------------------------------#
# Make a forest cover mask - 2 if developed upland, 1 if other cover, 0 if not 
#-----------------------------------------------------------------------------#

#----- Always included classes. Mark with a 2 --------------------------------#
good_codes <- grep("Forest", evc_codes$CLASSNAMES)
mat <- matrix(c(evc_codes$VALUE[good_codes], rep(2, length(good_codes))), 
              nrow=length(good_codes), ncol=2)

#----- Forest cover - conditional inclusion. Mark with a 1 -------------------#
good_codes <- grep("Tree Cover = 2.*", evc_codes$CLASSNAMES)
...
mat <- rbind(mat,
             matrix(c(evc_codes$VALUE[good_codes], rep(1, length(good_codes))), 
              nrow=length(good_codes), ncol=2))
...
# Values of 3 and 2 are forest, 1s are not
mat <- matrix(c(2:3, 1,1), nrow=2)
all <- classify(all, mat, others=NA)
...
```
```{r}
dff <- rast("../western_us_frg_grass.tif")
...
```
```{r}
dff <- rast("../western_us_frg_shrub.tif")
...
writeRaster(evc_out, "../temp_rasters/evc_excluded.tif", overwrite=T)
...
writeRaster(lcms_out, "../temp_rasters/lcms_excluded.tif", overwrite=T)
...
Here are ecoregion-level tables of FRP.
...
```{r calculate-mean-historical-fri-ecoregion}
#| eval: false
fri <- rast("../west_us_long_FRI.tif")
eco <- vect("../Data/level_3_ecoregion/eco_region.shp")
eco <- project(eco, fri)

#----- Total mean FRI by ecoregion -------------------------------------------#
vt <- exact_extract(fri, sf::st_as_sf(eco), fun='mean')
#forest_dat <- data.frame(Name = eco$NA_L3NAME, 
#                         Total_Area = vv,
#                         Area_Burned = v2)
```
