...
#reference-doc: style-reference.docx In an attempt to adjust font sizes - not worth it
editor: source
author: Lora Murphy
date: today
...
defi <- rast("../western_us_fire_deficit.tif")
fors <- rast("../western_us_forest.tif")

fordef <- mask(defi, fors)
writeRaster(fordef, "../forest_deficit.tif", overwrite=T)
```

```{r}
fordef <- rast("../forest_deficit.tif")
plot(fordef, axes=F,
     range=c(-1,1),
     fill_range=T,
     #col=map.pal("bgyr", bias=2.5),
...
defi <- rast("../western_us_fire_deficit.tif")
fors <- rast("../western_us_grass.tif")
...
defi <- rast("../western_us_fire_deficit.tif")
fors <- rast("../western_us_shrub.tif")
...
sum_fri <- exact_extract(fri, sf::st_as_sf(eco), fun='sum')
...
#----- Sum of FRI by ecoregion -----------------------------------------------#
sum_fri <- exact_extract(good_for, sf::st_as_sf(eco), fun='sum')
...
#----- Sum of FRI by ecoregion -----------------------------------------------#
sum_fri <- exact_extract(good_gr, sf::st_as_sf(eco), fun='sum')
...
#----- Sum of FRI by ecoregion -----------------------------------------------#
sum_fri <- exact_extract(good_shrub, sf::st_as_sf(eco), fun='sum')
