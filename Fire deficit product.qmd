...
A value of 1 indicates forest according to our definition, NA for all others. Our definition of forest is a pixel that is forest in both [Landfire EVC](https://landfire.gov/vegetation/evc) and the Landscape Change Monitoring System (LCMS) 2016 data. I received LCMS data from Mahsa.
...
lcms_crop <- project(lcms_crop, study_domain, method="near")

writeRaster(lcms_crop, "../temp_rasters/lcms_crop.tif", overwrite=T)
```
...
#----- Values of 2 are forest (both rasters agree), 1s are not ---------------#
...
LCMS forest cover class: 
-   GRASS/FORB/HERB
-   GRASS/FORB/HERB AND TREES MIX
-   GRASS/FORB/HERB AND SHRUBS MIX 
...
lcms_crop <- classify(lcms_crop, 
      cbind(c(10, # GRASS/FORB/HERB
              4,  # GRASS/FORB/HERB AND TREES MIX and 
              8   # GRASS/FORB/HERB AND SHRUBS MIX
  ), rep(1,3)), others=NA)
...
lcms_crop <- rast("../temp_rasters/lcms_crop.tif")
lcms_crop <- classify(lcms_crop, 
      cbind(c(3, # SHRUBS AND TREES MIX
              6, # TALL SHRUBS
              7  # SHRUBS
  ), rep(1,3)), others=NA)
