...
# Need more discussion on how to do this. The ecoregions are combined for some
# SPARKs.
#spark_l4 <- terra::vect("../../Spark counties shapefile/spark_l4_ecoregions.shp")
#spark_l4 <- terra::project(spark_l4, fri)

# Need to add SPARK names
#spark_l4 <- terra::aggregate(spark_l4, by="US_L4NAME")
#xx <- terra::relate(spark_l4, spark, "overlaps")
#spark_l4$SPARK <- ""
#for (i in 1:nrow(xx)) {
#  sparkname <- unique(spark$SPARK[xx[i,]])
#  sparkname <- sparkname[!is.na(sparkname)]
#  if (length(sparkname) != 1) stop("here")
#}
...
write.csv(summary_table_dat, "SPARK stats counties.csv", row.names=F)
...
all_sparks <- unique(spark_l4$SPARK)
all_sparks <- all_sparks[!is.na(all_sparks)]
all_sparks <- all_sparks[-grep("AK -", all_sparks)]
