...
I am calling "catch-up burning" the amount of burning required to restore the historical fire return interval, where multiple burns may be required. I am calling "reset burning" the amount of treating required, where any burning in an area of deficit resets the clock, and multiple burns will only be required in areas with very short fire return intervals to maintain balance.

My basic method for both is to calculate expected burns in 10 years for each area, with the appropriate method depending on whether the FRI is longer or shorter, and compare with the amount of burning so far. For catchup burns, we just use the difference. For reset burns, we use the difference, capped at 1. 

**Note on results:** For vegetation type / region combos where much of the area is in a long FRI area, where we make ecoregion-level calculations, the amount of burning (either catchup or reset) might be much less than the area in deficit. That's because, when an ecoregion is in deficit, the whole thing is in deficit; but it may be the case that only a small percentage of the area may need to burn to pull the whole thing back into balance. Our catchup and reset calculations take this into account.
